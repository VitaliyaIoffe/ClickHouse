...
def test_datetime_with_timezone(started_cluster):
    ...
    node1.query(
        "create table test_timezone ( ts DateTime, ts_z DateTime('America/New_York')) ENGINE PostgreSQL('postgres1:5432', 'postgres', 'test_timezone', 'postgres', 'mysecretpassword');"
    )
    ...
    assert (
        node1.query("select * from test_timezone")
...
def test_postgres_on_conflict(started_cluster):
    ...
    node1.query(
        f""" INSERT INTO {table} SELECT number, concat('name_', toString(number)), 3 from numbers(100)"""
    )
    node1.query(
        f""" INSERT INTO {table} SELECT number, concat('name_', toString(number)), 4 from numbers(100)"""
    )
