...
def create_replication_slot(conn, slot_name="user_slot"):
    cursor = conn.cursor()
    cursor.execute(
        "CREATE_REPLICATION_SLOT {} LOGICAL pgoutput EXPORT_SNAPSHOT".format(slot_name)
    )
    ...
    if len(schema_name) == 0:
        ...
        instance.query(
            """
                CREATE DATABASE {}
                ENGINE = PostgreSQL('{}:{}', '{}', 'postgres', 'mysecretpassword', '{}')""".format(
                name, ip, port, database_name, schema_name
            )
        )
...
def drop_postgres_table_with_schema(cursor, schema_name, table_name):
    cursor.execute(
        """DROP TABLE IF EXISTS "{}"."{}" """.format(schema_name, table_name)
    )
...
def assert_nested_table_is_created(
    ...
    database_tables = instance.query(
        "SHOW TABLES FROM {}".format(materialized_database)
    )
...
def check_tables_are_synchronized(
    ...
    if len(schema_name) == 0:
        result = instance.query(
            "select * from {}.{} order by {};".format(
                materialized_database, table_name, order_by
            )
        )
    else:
        result = instance.query(
            "select * from {}.`{}.{}` order by {};".format(
                materialized_database, schema_name, table_name, order_by
            )
        )
...
def test_load_and_sync_all_database_tables(started_cluster):
    ...
    for i in range(NUM_TABLES):
        ...
        instance.query(
            "INSERT INTO postgres_database.{} SELECT number, number from numbers(50)".format(
                table_name
            )
        )
...
def test_load_and_sync_subset_of_database_tables(started_cluster):
    ...
    for i in range(NUM_TABLES):
        ...
        create_postgres_table(cursor, "postgresql_replica_{}".format(i))
        instance.query(
            "INSERT INTO postgres_database.postgresql_replica_{} SELECT number, number from numbers(50)".format(
                i
            )
        )
...
def test_clickhouse_restart(started_cluster):
    ...
    instance.query(
        "CREATE DATABASE test_database ENGINE = MaterializedPostgreSQL('postgres1:5432', 'postgres_database', 'postgres', 'mysecretpassword')"
    )
...
def test_many_concurrent_queries(started_cluster):
    ...
    for i in range(NUM_TABLES):
        create_postgres_table(cursor, "postgresql_replica_{}".format(i))
        instance.query(
            "INSERT INTO postgres_database.postgresql_replica_{} SELECT number, number from numbers(10000)".format(
                i
            )
        )
    n = [10000]
...
def test_virtual_columns(started_cluster):
    ...
    assert_nested_table_is_created("postgresql_replica_0")
    instance.query(
        "INSERT INTO postgres_database.postgresql_replica_0 SELECT number, number from numbers(10)"
    )
    check_tables_are_synchronized("postgresql_replica_0")

    # just check that it works, no check with `expected` because _version is taken as LSN, which will be different each time.
    result = instance.query(
        "SELECT key, value, _sign, _version FROM test_database.postgresql_replica_0;"
    )
    print(result)

    cursor.execute("ALTER TABLE postgresql_replica_0 ADD COLUMN value2 integer")
    instance.query(
        "INSERT INTO postgres_database.postgresql_replica_0 SELECT number, number, number from numbers(10, 10)"
    )
    ...
    instance.query(
        "INSERT INTO postgres_database.postgresql_replica_0 SELECT number, number, number from numbers(20, 10)"
    )
    check_tables_are_synchronized("postgresql_replica_0")
...
def test_abrupt_connection_loss_while_heavy_replication(started_cluster):
    ...
    for i in range(NUM_TABLES):
        result = instance.query(
            "SELECT count() FROM test_database.postgresql_replica_{}".format(i)
        )
...
def test_drop_database_while_replication_startup_not_finished(started_cluster):
    ...
    for i in range(NUM_TABLES):
        ...
        create_postgres_table(cursor, table_name)
        instance.query(
            "INSERT INTO postgres_database.{} SELECT number, number from numbers(100000)".format(
                table_name
            )
        )
...
def test_user_managed_slots(started_cluster):
    ...
    create_materialized_db(
        ...
        port=started_cluster.postgres_port,
        settings=[
            "materialized_postgresql_replication_slot = '{}'".format(slot_name),
