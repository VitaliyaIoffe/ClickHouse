...
def test_executable_function_no_input(started_cluster):
    ...
    assert (
        node.query(
            "SELECT * FROM executable('test_no_input.sh', 'TabSeparated', 'value UInt64')"
        )
        == "1\n"
    )
...
def test_executable_function_input(started_cluster):
    ...
    assert (
        node.query(
            "SELECT * FROM executable('test_input.sh', 'TabSeparated', 'value String', (SELECT 1))"
        )
        == "Key 1\n"
    )
...
def test_executable_function_input_multiple_pipes(started_cluster):
    skip_test_msan(node)
    actual = node.query(
        "SELECT * FROM executable('test_input_multiple_pipes.sh', 'TabSeparated', 'value String', (SELECT 1), (SELECT 2), (SELECT 3))"
    )
...
def test_executable_function_argument(started_cluster):
    ...
    assert (
        node.query(
            "SELECT * FROM executable('test_argument.sh 1', 'TabSeparated', 'value String')"
        )
        == "Key 1\n"
    )
...
def test_executable_storage_no_input(started_cluster):
    ...
    node.query(
        "CREATE TABLE test_table (value UInt64) ENGINE=Executable('test_no_input.sh', 'TabSeparated')"
    )
...
def test_executable_storage_input(started_cluster):
    ...
    node.query("DROP TABLE IF EXISTS test_table")
    node.query(
        "CREATE TABLE test_table (value String) ENGINE=Executable('test_input.sh', 'TabSeparated', (SELECT 1))"
    )
...
def test_executable_storage_input_multiple_pipes(started_cluster):
    ...
    node.query("DROP TABLE IF EXISTS test_table")
    node.query(
        "CREATE TABLE test_table (value String) ENGINE=Executable('test_input_multiple_pipes.sh', 'TabSeparated', (SELECT 1), (SELECT 2), (SELECT 3))"
    )
...
def test_executable_storage_argument(started_cluster):
    ...
    node.query("DROP TABLE IF EXISTS test_table")
    node.query(
        "CREATE TABLE test_table (value String) ENGINE=Executable('test_argument.sh 1', 'TabSeparated')"
    )
    assert node.query("SELECT * FROM test_table") == "Key 1\n"
    node.query("DROP TABLE test_table")
...
def test_executable_pool_storage(started_cluster):
    ...
    node.query("DROP TABLE IF EXISTS test_table")
    node.query(
        "CREATE TABLE test_table (value String) ENGINE=ExecutablePool('test_input_process_pool.sh', 'TabSeparated', (SELECT 1))"
    )
    assert node.query("SELECT * FROM test_table") == "Key 1\n"
    node.query("DROP TABLE test_table")
...
def test_executable_pool_storage_multiple_pipes(started_cluster):
    ...
    node.query("DROP TABLE IF EXISTS test_table")
    node.query(
        "CREATE TABLE test_table (value String) ENGINE=ExecutablePool('test_input_process_pool_multiple_pipes.sh', 'TabSeparated', (SELECT 1), (SELECT 2), (SELECT 3))"
    )
    assert (
