...
def feature(self, node="clickhouse1"):
    ...
    try:
        ...
        with Scenario(
            "I show create row policy",
            requirements=[RQ_SRS_006_RBAC_RowPolicy_ShowCreateRowPolicy("1.0")],
        ):
        ...
        with Scenario(
            "I show create row policy on a table",
            requirements=[RQ_SRS_006_RBAC_RowPolicy_ShowCreateRowPolicy_On("1.0")],
        ):
            with cleanup("policy0"):
                with When("I run show create row policy command"):
                    node.query("SHOW CREATE ROW POLICY policy0 ON default.foo")
        ...
        with Scenario(
            "I show create row policy using short syntax on a table",
            requirements=[RQ_SRS_006_RBAC_RowPolicy_ShowCreateRowPolicy_On("1.0")],
        ):
