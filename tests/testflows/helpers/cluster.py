...
class Node(object):
    ...
    def stop(self, timeout=300, retries=5, safe=True):
        ...
        for retry in range(retries):
            r = self.cluster.command(
                None, f"{self.cluster.docker_compose} stop {self.name}", timeout=timeout
            )
            if r.exitcode == 0:
                break
    ...
    def wait_healthy(self, timeout=300):
        with By(f"waiting until container {self.name} is healthy"):
            ...
            while True:
                if (
                    self.query(
                        "select 1", no_checks=1, timeout=300, steps=False
                    ).exitcode
                    == 0
                ):
                    break
...
class Cluster(object):
    ...
    def __init__(
        ...
        if not os.path.exists(docker_compose_file_path):
            raise TypeError(
                "docker compose file '{docker_compose_file_path}' does not exist"
            )
    ...
    def down(self, timeout=300):
        ...
        try:
            ...
            cmd = self.command(
                None,
                f"{self.docker_compose} down --timeout 60",
                bash=bash,
                timeout=timeout,
            )
            with self.lock:
