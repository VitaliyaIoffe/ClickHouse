...
class PRInfo:
    def __init__(self, github_event, need_orgs=False, need_changed_files=False):
        if "pull_request" in github_event:  # pull request and other similar events
            ...
            if (
                pull_request is None or pull_request["state"] == "closed"
            ...
            if need_changed_files:
                if self.number == 0:
                    ...
                    response = requests.get(
                        f"https://api.github.com/repos/{os.getenv('GITHUB_REPOSITORY')}/compare/{commit_before}...{self.sha}"
                    )
                    response.raise_for_status()
                    diff = response.json()
