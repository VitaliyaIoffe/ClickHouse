...
class ClickhouseIntegrationTestsRunner:
    ...
    def run_test_group(
        ...
        for i in range(num_tries):
            ...
            if (
                ...
                logging.info(
                    "Seems like all tests passed but some of them are skipped or deselected. Ignoring them and finishing group."
                )
                break
        else:
    ...
    def run_impl(self, repo_path, build_path):
        ...
        for group, tests in items_to_run:
            logging.info(
                "Running test group %s countaining %s tests", group, len(tests)
            )
