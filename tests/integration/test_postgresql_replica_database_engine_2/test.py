...
def test_add_new_table_to_replication(started_cluster):
    ...
    for i in range(NUM_TABLES):
        ...
        instance.query(
            "INSERT INTO postgres_database.postgresql_replica_{} SELECT number, {} from numbers(10000)".format(
                i, i
            )
        )
    ...
    result = instance.query_and_get_error(
        "ATTACH TABLE test_database.{}".format(table_name)
    )
...
def test_remove_table_from_replication(started_cluster):
    ...
    result = instance.query_and_get_error(
        "SELECT * FROM test_database.{}".format(table_name)
    )
...
def test_database_with_single_non_default_schema(started_cluster):
    ...
    check_tables_are_synchronized(
        f"postgresql_replica_{altered_table}", postgres_database=clickhouse_postgres_db
    )
    drop_materialized_db()
...
def test_database_with_multiple_non_default_schemas_2(started_cluster):
    ...
    assert_number_of_columns(
        3, f"schema{altered_schema}.postgresql_replica_{altered_table}"
    )
    check_tables_are_synchronized(
        f"postgresql_replica_{altered_table}",
        schema_name=schema_name,
