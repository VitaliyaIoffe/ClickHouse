...
if __name__ == "__main__":
    ...
    if retcode != 0:
        commit.create_status(
            context=NAME,
            description="PVS report failed to build",
            state="failure",
            target_url=f"https://github.com/ClickHouse/ClickHouse/actions/runs/{os.getenv('GITHUB_RUN_ID')}",
        )
        sys.exit(1)
    ...
    try:
        ...
        html_urls = s3_helper.upload_test_folder_to_s3(
            os.path.join(temp_path, HTML_REPORT_FOLDER), s3_path_prefix
        )
        index_html = None
        ...
        status = "success"
        test_results = [
            (index_html, "Look at the report"),
            ("Errors count not checked", "OK"),
        ]
        description = "Total errors {}".format(len(errors))
        additional_logs = [txt_report, os.path.join(temp_path, "pvs-studio.log")]
        report_url = upload_results(
            s3_helper, pr_info.number, pr_info.sha, test_results, additional_logs, NAME
        )
