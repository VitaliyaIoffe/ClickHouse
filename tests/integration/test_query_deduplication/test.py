...
def prepare_node(node, parts_uuid=None):
    ...
    print(
        node.query(
            "SELECT name, uuid, partition FROM system.parts WHERE table = 't' AND active ORDER BY name"
        )
    )
...
"""
    assert TSV(expected) == TSV(
        node1.query(
            "SELECT key, count() c FROM d GROUP BY key ORDER BY key",
            settings={
