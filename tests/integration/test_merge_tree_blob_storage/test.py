...
def test_read_after_cache_is_wiped(cluster):
    ...
    assert (
        node.query(f"SELECT * FROM {TABLE_NAME} order by dt, id FORMAT Values")
        == values
    )
...
def test_inserts_selects(cluster):
    ...
    assert (
        node.query(f"SELECT count(*) FROM {TABLE_NAME} where id = 1 FORMAT Values")
        == "(2)"
    )
...
def test_insert_same_partition_and_merge(cluster, merge_vertical):
    ...
    assert (
        node.query(f"SELECT count(distinct(id)) FROM {TABLE_NAME} FORMAT Values")
        == "(8192)"
    )
...
def test_alter_table_columns(cluster):
    ...
    assert (
        node.query(f"SELECT sum(col1) FROM {TABLE_NAME} WHERE id > 0 FORMAT Values")
        == "(4096)"
    )
    ...
    assert (
        node.query(f"SELECT distinct(col1) FROM {TABLE_NAME} FORMAT Values") == "('1')"
    )
...
def test_move_partition_to_another_disk(cluster):
    ...
    node.query(
        f"ALTER TABLE {TABLE_NAME} MOVE PARTITION '2020-01-04' TO DISK '{BLOB_STORAGE_DISK}'"
    )
    assert node.query(f"SELECT count(*) FROM {TABLE_NAME} FORMAT Values") == "(8192)"
...
def test_move_replace_partition_to_another_table(cluster):
    ...
    assert (
        node.query(f"SELECT count(*) FROM {table_clone_name} FORMAT Values") == "(512)"
    )
...
def test_restart_during_load(cluster):
    ...
    def read():
        for ii in range(0, 5):
            ...
            assert (
                node.query(f"SELECT sum(id) FROM {TABLE_NAME} FORMAT Values") == "(0)"
            )
